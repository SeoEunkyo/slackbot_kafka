...
// GenerateTimeLoanModal
// 단축근무 모달
func GenerateTimeLoanModal() slack.ModalViewRequest {
	// Create a ModalViewRequest with a header and two inputs
	titleText := slack.NewTextBlockObject("plain_text", "로동시간 대출 시전", false, false)
	closeText := slack.NewTextBlockObject("plain_text", "Close", false, false)
	submitText := slack.NewTextBlockObject("plain_text", "Submit", false, false)

	headerText := slack.NewTextBlockObject("mrkdwn", "동무의 로동시간을 대출, 사용합니다.(최대 +-4시간, 하루 1시간이내)", false, false)
	...
	breakTimeLabel := slack.NewTextBlockObject("plain_text", "지녁식사시간(분)", false, false)
	...
	modalRequest.CallbackID = "time_loan"
